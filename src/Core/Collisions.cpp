...
namespace core {
    BBOXCollision check_collision_boxes(const BoundingBox &box1, const BoundingBox &box2) {
        ...
        Vector3 box1_dims = Vector3Scale(utils::get_bounding_box_dimensions(box1), 0.5f);
        Vector3 box2_dims = Vector3Scale(utils::get_bounding_box_dimensions(box2), 0.5f);

        Vector3 delta = Vector3Subtract(box1_center, box2_center);

        float overlap_x = box1_dims.x + box2_dims.x - fabsf(delta.x);
        float overlap_y = box1_dims.y + box2_dims.y - fabsf(delta.y);
        float overlap_z = box1_dims.z + box2_dims.z - fabsf(delta.z);

        if (overlap_x <= EPSILON || overlap_y <= EPSILON || overlap_z <= EPSILON) {
